...
class Complexity_labeller:
	...
	def convert_format_string(string):
	    ...
	    dataframe['binary'] = 'N'
	    dataframe.to_csv(temp_file, sep = '\t',index=False, header=False, quotechar=' ') 
	    
	def convert_format_token(tokenised_string):
	    dataframe = pd.DataFrame()
	...
	def convert_rasp_table(rasp_table):
